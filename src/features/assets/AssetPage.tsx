  ...
  return (
    <div className={`fy-screen fy-asset-screen ${frozen ? 'fy-is-frozen' : ''}`}>
      <BlurFade><GlassCard className="fy-asset-header">
        ...
        <div className="fy-asset-title"><h1>{player.name}</h1><div><Badge>{player.ticker}</Badge> <Badge>{player.league}</Badge> {player.status && player.status !== 'Open' && <Badge variant={player.status.includes('Risk') ? 'danger' : 'warning'}>{player.status}</Badge>} {retired && <Badge>RETIRED ★</Badge>}</div></div>
        <div className="fy-asset-price"><strong className="fy-pixel"><CurrencyAmount>{player.price.toFixed(2)}</CurrencyAmount></strong>{player.change == null ? <Badge>Change unavailable</Badge> : <BadgeDelta value={`${Math.abs(player.change)}%`} deltaType={getDeltaType(player.change)} />}</div>
        <div className="fy-mini-stats"><div><span>Market price</span><strong><CurrencyAmount>{player.price.toFixed(2)}</CurrencyAmount></strong></div><div><span>Price change</span><strong>{player.change == null ? 'Not available' : `${player.change}%`}</strong></div><div><span>Market</span><strong>{player.status || 'Not available'}</strong></div></div>
      ...
      <div className="fy-asset-layout">
        <div className="fy-asset-main">
          <BlurFade delay={0.08}><GlassCard className="fy-chart-card"><CardTitle title="Price Chart" /><div className="fy-empty"><strong>Price history unavailable</strong><span>Historical points will appear when the market service publishes them.</span></div></GlassCard></BlurFade>
          <BlurFade delay={0.12}><GlassCard><CardTitle title="Statistics" /><div className="fy-empty"><strong>No additional statistics</strong><span>Sports statistics are not available for this asset yet.</span></div></GlassCard></BlurFade>
          <BlurFade delay={0.16}><GlassCard><CardTitle title="Order Book" /><div className="fy-empty"><strong>Order book unavailable</strong><span>Live depth will appear when the Market Engine exposes it.</span></div></GlassCard></BlurFade>
        </div>
