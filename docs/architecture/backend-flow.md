...
The application registers strict CORS for exactly
`https://field-yield.vercel.app`, allowing credentials, all methods, and all
request headers. A request logging middleware records method, URL, and status
...
`FRONTEND_URL` is validated against the single production origin. `AUTH_PROVIDER`
is either `supabase` or `local`; it never silently falls back. Supabase mode
fails closed when its URL/anon key is absent.

`GET /health` checks the SQL connection, checks Supabase Auth when configured,
and checks the optional Market Engine readiness endpoint when a base URL is
present. It returns `status: ok` when the database is available and the
selected auth provider is ready. The `supabase` and `market_engine` fields
...
- `current_user` requires a bearer token. In Supabase mode it calls
  `<SUPABASE_URL>/auth/v1/user` with the public anon key and bearer token,
  extracts the verified Supabase user ID/provider/email, then looks up the
  exact `users.auth_provider_id`. In local mode it verifies the signed local
  JWT subject.
...
| Squad | `/api/v1/squad*` | Ownership check, 25-player cap, unique player/position constraints |
...
Trading in the current V1 is a local SQL market-order service. It uses current
...
`AUTH_PROVIDER`, `FRONTEND_URL`, `SIGNUP_BONUS_ENABLED`,
`SIGNUP_BONUS_GOLD`, `SIGNUP_BONUS_SILVER`, `SUPABASE_URL`,
`SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `ADMIN_EMAILS`,
`MARKET_ENGINE_BASE_URL`, `MARKET_ENGINE_API_KEY`,
`MARKET_ENGINE_TIMEOUT_MS`, `MARKET_ENGINE_VERSION`, and `ALLOW_TEST_CREDIT`
are server variables. Keep them in Render's environment, never in the Vite
bundle. `SUPABASE_SERVICE_ROLE_KEY` is reserved for future server-side admin
integrations and is not sent to browsers.
