...
| `POST` | `/v1/users/{user_id}/bootstrap` | Engine-side wallet/position bootstrap. |
...
endpoint, a FieldYield wallet reconciliation endpoint, or a signed service
identity contract. Therefore the adapter is health/order/cancellation capable,
but it is **not enabled as the authoritative settlement path**. FieldYield's
SQL transaction service remains authoritative and the external adapter is only
used when an explicitly integrated feature calls it.
...
6. A reconciliation endpoint or signed event stream for fills, balances, and holdings.
...
remain on the FieldYield SQL path and the Market Engine health endpoint should
be treated as an integration diagnostic, not proof that settlement is safe.
